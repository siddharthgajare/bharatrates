...
import { STATES, normalizeState, type StateInfo } from '@/lib/states';
import {
  CATEGORY_COLORS,
  CATEGORY_LABELS,
...
export default function StateMaterialsClient({ stateSlug }: { stateSlug: string }) {
  const stateInfo = useMemo<StateInfo | null>(() => normalizeState(stateSlug), [stateSlug]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!hasToday) {
          await triggerGeneration(stateInfo.name);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `${stateInfo?.name.toLowerCase().replace(/[^a-z]+/g, '-')}-prices-${updatedAt ?? 'latest'}.csv`;
    a.click();
  ...
  const exportJSON = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${stateInfo?.name.toLowerCase().replace(/[^a-z]+/g, '-')}-prices-${updatedAt ?? 'latest'}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 text-sm text-primary mb-2">
              ...
              {stateInfo.region.charAt(0).toUpperCase() + stateInfo.region.slice(1)} India
            </div>
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">{stateInfo.name}</h1>
            <p className="text-muted-foreground mt-1.5 text-sm">
              Capital {stateInfo.capital} · {stateInfo.cities.length} cities tracked · 4 material categories
            </p>
          </div>
